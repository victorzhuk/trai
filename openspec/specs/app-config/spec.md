...
The operator's single configuration surface: the TOML file the application reads at startup, the keys it carries, and the validation that refuses to start on a misconfiguration by naming the offending key. It exists so everything a Recording depends on — Store root, capture sources, Segment cutting, the whisper endpoint and its dialect, the confidence floor, the translate backends — is set in one file instead of in code.
